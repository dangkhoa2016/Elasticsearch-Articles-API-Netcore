...
namespace elasticsearch_netcore.Repositories
{
    ...
    {
        Task<dynamic> GetAuthors(int skip = 0, int take = AppConstants.DefaultPageSize
            , Expression<Func<Author, bool>> filter = null, bool showTotal = false);
        Task<dynamic> GetAuthors(int skip = 0, int take = AppConstants.DefaultPageSize, string name = "", bool showTotal = false);
        ...
        Task<dynamic> GetArticlesForAuthor(long id, int skip = 0, int take = AppConstants.DefaultPageSize, string title = "",
